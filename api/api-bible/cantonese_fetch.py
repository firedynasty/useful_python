...
Cantonese TTS using macOS Sinji voice.
Usage: python cantonese_fetch.py john 3:16
       python cantonese_fetch.py --speak john 3:16
"""
...
def speak_text(text):
    """
    Speak text using macOS say command with Cantonese Sinji voice.
    """
    if not text:
        return
    ...
    for i in range(0, text_length, chunk_size):
        ...
        try:
            # Use macOS say with Cantonese Sinji voice at rate 120
            say_proc = subprocess.Popen(
                ["/usr/bin/say", "-v", "Sinji", "-r", "120", chunk, "-o", "/tmp/temp.aiff"],
            ...
            ffmpeg_proc = subprocess.Popen(
                ["/opt/homebrew/bin/ffmpeg", "-y", "-i", "/tmp/temp.aiff", "/tmp/temp.mp3"],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
            ffmpeg_proc.wait()
            ...
            mpg123_proc = subprocess.Popen(
                ["/opt/homebrew/bin/mpg123", "-q", "/tmp/temp.mp3"],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
            mpg123_proc.wait()
            ...
            print("Make sure ffmpeg and mpg123 are installed.", file=sys.stderr)
            break
...
if __name__ == "__main__":
    ...
    if len(args) < 1:
        print("Usage: python cantonese_fetch.py [--speak] <book> <chapter>:<verse>", file=sys.stderr)
        print("Examples:", file=sys.stderr)
        print("  python cantonese_fetch.py john 3:16", file=sys.stderr)
        print("  python cantonese_fetch.py --speak john 3:16  (--speak will speak)", file=sys.stderr)
        print("  python cantonese_fetch.py psalm 23:1-6", file=sys.stderr)
        print("  python cantonese_fetch.py 1 john 1:9", file=sys.stderr)
        sys.exit(1)
    ...
    print(result['text'])
    print("cantonesespeak will speak")
