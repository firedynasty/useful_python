#!/bin/bash
# Full pipeline: segment audio, transcribe each segment, merge SRTs,
# create video (black background + burned lyrics + audio)
#
# Usage: bash run.sh <input.m4a>
#
# Example: bash run.sh "song.m4a"
#
# Automatically determines how many 10-min segments are needed from audio duration.

set -e

INPUT="${1:?Usage: bash run.sh <input.m4a>}"
...
echo "Audio duration: ${TOTAL_SEC}s => ${NUM_SEGS} segment(s) of ${SEGMENT_DURATION}s"
echo ""
...
for SEG in $(seq 1 "$NUM_SEGS"); do
  ...
  echo "=========================================="
  if ! bash "$SCRIPT_DIR/audio_subtitle.sh" "$FILENAME" "$SEG"; then
    EC=$?
    if [ "$NUM_SEGS" -eq 1 ]; then
      echo "Error: audio_subtitle.sh failed with exit code $EC on the only segment"
      exit $EC
    fi
...
# === Step 3: Create video with black background, burned lyrics, and audio ===
echo ""
echo "=== Creating lyric video (black background + subtitles + audio) ==="
cp english.srt /tmp/english_work.srt

# Generate black background video matching audio duration, burn subtitles, add audio
ffmpeg -y \
  -f lavfi -i "color=c=black:s=1920x1080:d=$TOTAL_SEC:r=25" \
  -i "$FILENAME" \
  -filter_complex \
  "[0:v]subtitles=filename=/tmp/english_work.srt:fontsdir=/Library/Fonts:force_style='FontName=Arial Unicode MS,FontSize=24,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,Outline=2,Alignment=2,MarginV=40'[v1]" \
  -map "[v1]" -map 1:a -c:v libx264 -preset medium -crf 23 -c:a aac -b:a 192k \
  -shortest \
...
echo "Lyric video:    ${DIR}/${BASENAME}_subtitled.mp4"
