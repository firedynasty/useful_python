"""
Karaoke-style video generator using ElevenLabs TTS + OpenAI Whisper API.

Step 1: Generate AI voiceover with ElevenLabs TTS
Step 2: Transcribe it back with OpenAI Whisper API (whisper-1) for word-level timestamps
Step 3: Render video with text and an underline indicator showing the current word

Requirements:
    pip install elevenlabs openai moviepy pillow numpy

Usage:
    python karaoke_elevenlabs.py --text sample_text.md --output output.mp4

Optional:
    --voice-id "21m00Tcm4TlvDq8ikWAM"  ElevenLabs voice ID (default: Rachel)
    --audio voiceover.mp3     Pre-generated audio file (skips TTS step)
...
def generate_elevenlabs_tts(text, voice_id="21m00Tcm4TlvDq8ikWAM", api_key=None):
    """Generate speech audio using ElevenLabs TTS API."""
    import base64
    from elevenlabs import ElevenLabs

    client = ElevenLabs(api_key=api_key or os.environ.get("ELEVEN_API_KEY"))
...
# Common ElevenLabs voice IDs (no API call needed):
...
def main():
    parser = argparse.ArgumentParser(description="Generate karaoke-style video with ElevenLabs TTS + Whisper")
    ...
    parser.add_argument("--voice-id", default="21m00Tcm4TlvDq8ikWAM",
                        help="ElevenLabs voice ID (default: Rachel). See voice ID list in script.")
    ...
    parser.add_argument("--api-key", default=None, help="ElevenLabs API key (or set ELEVEN_API_KEY env var)")
    parser.add_argument("--openai-api-key", default=None, help="OpenAI API key for Whisper (or set OPENAI_API_KEY env var)")
    ...
    # Step 1: Generate audio with ElevenLabs TTS (or use pre-generated)
    if args.audio:
        audio_path = args.audio
        print(f"Step 1: Skipped — using existing audio: {audio_path}")
    else:
        print("Step 1: Generating ElevenLabs TTS audio...")
        audio_path = generate_elevenlabs_tts(
            clean_text, voice_id=args.voice_id, api_key=args.api_key
        )
        print(f"Audio saved to: {audio_path}")

    # Step 2: Transcribe with Whisper API for word timestamps
    print("\nStep 2: Getting word timestamps with Whisper API...")
    word_timestamps = transcribe_with_whisper_api(audio_path, api_key=args.openai_api_key)
