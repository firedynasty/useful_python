#!/usr/bin/env python3
"""
Extract transcript segments one minute before or after a specified time.
Usage: python extract_transcript_segment.py transcript.txt 1:23 backward [custom_question]
       python extract_transcript_segment.py transcript.txt 1:23 forward [custom_question]
"""
...
def find_segment(entries: List[Tuple[int, str, str]], target_time: int, target_format: str, 
                direction: str = "backward", window: int = 60) -> List[Tuple[int, str]]:
    """Find transcript entries within window seconds before or after target time, filtering by format."""
...
def main():
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        print("Usage: python extract_transcript_segment.py transcript.txt MM:SS <backward|forward> [custom_question]")
        print("Example: python extract_transcript_segment.py transcript.txt 1:23 backward")
        print("Example: python extract_transcript_segment.py transcript.txt 1:23 forward")
        print('Example: python extract_transcript_segment.py transcript.txt 1:23 backward "What are the main themes?"')
        sys.exit(1)
    
    filename = sys.argv[1]
    target_time_str = sys.argv[2]
    direction = sys.argv[3].lower()
    custom_question = sys.argv[4] if len(sys.argv) == 5 else None
    
    if direction not in ["backward", "forward"]:
        print("Error: Direction must be 'backward' or 'forward'")
        sys.exit(1)
    
    try:
        target_time = parse_time(target_time_str)
        entries = extract_transcript_entries(filename)
        segment = find_segment(entries, target_time, target_time_str, direction)
        
        if not segment:
            print(f"No transcript entries found {direction} from {target_time_str}")
            return
        
        direction_label = "before" if direction == "backward" else "after"
        print(f"Transcript segment 1 minute {direction_label} {target_time_str}:")
        print("=" * 60)
        ...
        # Add question only for backward direction
        if direction == "backward":
            if custom_question:
            ...
            final_text = combined_text_with_question
        else:
            # For forward direction, just use the text (with custom question if provided)
            if custom_question:
        ...
        try:
            ...
            suffix = " with question" if (direction == "backward" or custom_question) else ""
            print(f"\n[Text{suffix} copied to clipboard]")
        except ImportError:
