...
def list_folder_files_with_urls(service, folder_id, folder_name=""):
    """List all files in a folder with their URLs."""
    try:
        items = get_folder_contents(service, folder_id)
        ...
        urls = []
        print(f"\nContents of folder '{folder_name}':" if folder_name else "\nFolder contents:")
        print("=" * 50)
        ...
        for file in files:
            ...
            print(f"{file_count}. 📄 {file['name']}{size_display}")
            print(f"   URL: {url}")
            print()
        ...
        if file_count == 0:
            print("No files found (only subfolders).")
        else:
...
def main():
    ...
    parser.add_argument('search_term', nargs='?', help='Folder or file name to search for, or a Google Drive URL')
    ...
    if 'drive.google.com' in args.search_term:
        folder_id = extract_folder_id_from_url(args.search_term)
        if folder_id:
            print(f"\nDetected Google Drive folder URL")
            ...
            # List files with URLs
            urls = list_folder_files_with_urls(service, folder_id, folder_name)
            ...
            if urls:
                ...
                while True:
                    if len(urls) == 1:
                        response = input("\nCopy file URL to clipboard? (y/q to quit), g opens copied url: ").lower().strip()
                        ...
                        response = input(f"\nCopy file URL to clipboard? Enter number (1-{len(urls)}), 'g' to open copied URL, or 'q' to quit: ").strip().lower()
                        if response in ['q', 'quit', 'exit']:
                            break
                        elif response == 'g':
                            if last_copied_url:
            ...
            print("Could not extract folder ID from URL. Proceeding with regular search...")
    ...
    if urls:
        ...
        while True:
            if len(urls) == 1:
                response = input("\nCopy URL to clipboard? (y/q to quit), g will open clipboard in chrome: ").lower().strip()
                if response in ['q', 'quit', 'exit']:
                    break
                elif response in ['y', 'yes']:
                    if PYPERCLIP_AVAILABLE:
