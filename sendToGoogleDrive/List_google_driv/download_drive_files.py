...
# Same read-only scope works for downloads
...
def download_full_file_as_text(service, file_id, mime_type):
    ...
    try:
        ...
        if mime_type.startswith('application/vnd.google-apps'):
            ...
            if mime_type in export_map:
                ...
                return "[Unsupported Google format for text extraction]"
...
def download_and_combine_files(service, files_list, output_file='combined_output.txt'):
    """Download files and combine them into a single output file with separators."""
    if not files_list:
        print("No files to download.")
        return

    # Filter supported file types for text extraction
    ...
    if not downloadable_files:
        print("No downloadable text files found (txt, md, csv, rtf, pdf, Google Docs).")
        return

    print(f"\nDownloading {len(downloadable_files)} file(s)...")
    print("=" * 60)

    file_contents = []

    for idx, file in enumerate(downloadable_files, 1):
        print(f"[{idx}/{len(downloadable_files)}] Downloading: {file['name']}")

        # Download full file content
        file_content = download_full_file_as_text(service, file['id'], file['mimeType'])

        # Add file header and content
        file_url = f"https://drive.google.com/file/d/{file['id']}/view"

        # Format: filename, URL, separator, content (similar to previewfiles.sh)
        file_entry = []
        file_entry.append(f"=== {file['name']} ===")
        file_entry.append(f"URL: {file_url}")
        file_entry.append("***_______")
        file_entry.append(file_content)

        file_contents.append('\n'.join(file_entry))

    # Join all content with separator between files
    separator = "###\n###"
    final_content = f"\n{separator}\n".join(file_contents)
    ...
    try:
        ...
        print("=" * 60)
        print(f"\n✓ Successfully combined {len(downloadable_files)} file(s)")
...
def main():
    """Main function to download and combine files from Google Drive."""
    import argparse

    parser = argparse.ArgumentParser(description='Download and combine files from Google Drive into a single text file')
    ...
    parser.add_argument('-o', '--output', default='combined_output.txt',
                       help='Output filename (default: combined_output.txt)')
        ...
        if item_id:
            ...
            try:
                ...
                if is_folder:
                    ...
                    files_to_download = get_folder_contents(service, item_id, recursive=recursive)
                else:
                    # It's a single file
                    files_to_download = [item]

                # Download and combine the files
                download_and_combine_files(service, files_to_download, args.output)
                return
    ...
    response = input(f"Download which item? (1-{len(results)}, 'all', or 'n' to cancel): ").strip().lower()
    ...
    if response == 'all':
        for item in results:
            if item['mimeType'] == 'application/vnd.google-apps.folder':
                files_to_download.extend(get_folder_contents(service, item['id'], recursive=args.recursive))
            else:
        ...
        if 0 <= idx < len(results):
            ...
            if item['mimeType'] == 'application/vnd.google-apps.folder':
                ...
                files_to_download.extend(get_folder_contents(service, item['id'], recursive=recursive))
            else:
                files_to_download.append(item)
        else:
