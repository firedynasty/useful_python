...
def main():
    ...
    # Interactive URL copying loop
    if PYPERCLIP_AVAILABLE and urls:
        print("=" * 50)
        while True:
            if len(urls) == 1:
                response = input("\nCopy URL to clipboard? (y/q to quit): ").lower().strip()
                ...
                response = input(f"\nCopy URL to clipboard? Enter number (1-{len(urls)}) or 'q' to quit: ").strip().lower()
                if response in ['q', 'quit', 'exit']:
                    break
                    ...
                    print(f"Invalid input. Enter a number (1-{len(urls)}) or 'q' to quit.")
