#!/usr/bin/env python3
"""
Transcribe Chinese text from image(s) using OpenAI GPT-4o Vision API.
...
Usage:
    # Single image
    python extract_images_openai_txt_chinese.py -i page0.png -o output.txt

    # Folder of screenshots
    python extract_images_openai_txt_chinese.py -i ./screenshots -o output.txt

    # Folder → text + also save as PDF
    python extract_images_openai_txt_chinese.py -i ./screenshots -o output.txt --pdf combined.pdf
...
def run(input_path: str, output_file: str | None, pdf_path: str | None, prompt: str, clipboard: bool):
    try:
    ...
    # Ensure output directory exists and clear any existing file
    if output_file:
        out = Path(output_file)
        if out.parent != Path('.'):
            out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text('', encoding='utf-8')  # clear file before appending
    ...
    for i, img_path in enumerate(image_paths):
        ...
        try:
            ...
            if i > 0:
                ...
                block = f"PAGE 1 — {img_path.name}\n{'='*50}\n\n{text}\n"

            all_text_parts.append(block)

            # Write incrementally to file if requested
            if output_file:
    ...
    if clipboard:
        copy_to_clipboard(''.join(all_text_parts))
    if output_file:
...
def main():
    parser = argparse.ArgumentParser(
        description='Transcribe Chinese text from image(s) using OpenAI GPT-4o Vision'
    )
    ...
    parser.add_argument('-c', '--clipboard', action='store_true',
                        help='Copy extracted text to clipboard instead of (or in addition to) -o')
    parser.add_argument('--pdf', default=None, metavar='PDF_PATH',
                        help='Also save all images as a multi-page PDF at this path')
    parser.add_argument('--prompt', default=(
        'Please transcribe ALL Chinese text visible in this image exactly as it appears. '
        'Preserve the original Chinese characters (do not convert to pinyin or translate). '
        'Keep line breaks and punctuation. '
        'If there is any English text also include it as-is. '
    ...
    if not args.output and not args.clipboard:
        parser.error('Specify -o <file> to save output, -c to copy to clipboard, or both.')

    run(args.input, args.output, args.pdf, args.prompt, args.clipboard)
