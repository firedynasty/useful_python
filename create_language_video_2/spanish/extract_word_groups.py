...
Generates a CSV showing all Spanish CEFR words grouped by level and lesson.
...
for level in ALL_LEVELS:
    ...
    for lesson in range(1, total_lessons + 1):
        ...
        for w in batch:
            ...
            pos = w.get("pos", "")
            gender = w.get("gender", "")
            rows.append([level, lesson, spanish, pos, gender or "", english])

with open(args.output, "w", newline="", encoding="utf-8-sig") as f:
    writer = csv.writer(f)
    writer.writerow(["CEFR Level", "Lesson", "Spanish", "POS", "Gender", "English"])
