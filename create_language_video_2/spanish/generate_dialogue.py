...
Loads Spanish CEFR vocabulary from JSON files, splits a level into lesson
...
for w in batch:
    ...
    focus_lines.append(f"  {spanish}{gender_str} [{pos}] — {english}")

focus_list = "\n".join(focus_lines)

# ── Generate dialogue ──────────────────────────────────────────────────────

prompt = f"""Write a natural Spanish conversation between two people.
...
- Use natural spoken Spanish (contractions, informal register where appropriate for the level)
    ...
    messages=[
        {
            "role": "system",
            "content": (
                "You are a Spanish language teaching assistant. "
