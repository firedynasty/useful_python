...
Reads a dialogue.txt (A: target language, B: English pairs),
generates per-line audio via OpenAI TTS, stitches with pydub,
and generates target.srt + english.srt with timestamps.
...
while i < len(raw):
    ...
    if ma:
        # Try A/P/B triplet first, fall back to A/B pair
        pinyin = None
        next_idx = i + 1
        if next_idx < len(raw):
...
short_pause = AudioSegment.silent(duration=400)  # between target and English
full_audio = AudioSegment.empty()

srt_target = []
srt_pinyin = []
srt_english = []
has_pinyin = any("pinyin" in e for e in entries)
cursor_ms = 0
...
for i, entry in enumerate(entries):
    ...
    # SRT: both target and English show for the full exchange duration
    idx = i + 1
    srt_target.append(f"{idx}\n{fmt_srt(target_start)} --> {fmt_srt(eng_end)}\n{entry[TARGET_FIELD]}\n")
    if has_pinyin:
        pinyin_text = entry.get("pinyin", "")
        srt_pinyin.append(f"{idx}\n{fmt_srt(target_start)} --> {fmt_srt(eng_end)}\n{pinyin_text}\n")
...
if has_pinyin:
    pinyin_path = os.path.join(args.output_dir, "pinyin.srt")
