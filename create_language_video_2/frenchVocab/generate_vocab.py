...
Builds CEFR-level French vocabulary JSONs from a frequency list (fr_50k.txt)
and uses OpenAI only for English translations + POS/gender tagging.

1. Parses fr_50k.txt (hermitdave/FrequencyWords — OpenSubtitles frequency data)
2. Filters out junk (contractions, single letters, numbers)
...
parser.add_argument("--freq_file", default="fr_50k.txt", help="Frequency list file")
...
# Patterns to skip
SKIP_RE = re.compile(
    r"^[a-z]?'$"       # bare contractions: c', l', d', j', n', s', qu'
    r"|^\d+$"           # pure numbers
    r"|^.$"             # single characters
    r"|^-+$"            # dashes
...
with open(args.freq_file, "r", encoding="utf-8") as f:
    for line in f:
        ...
        words.append({"french": word, "frequency": freq})
...
def translate_batch(french_words):
    """Send a batch of French words to OpenAI for translation + POS + gender."""

    word_list = "\n".join(f"  {i+1}. {w}" for i, w in enumerate(french_words))

    prompt = f"""For each French word below, provide: English translation, part of speech, and gender (for nouns).

{word_list}

Return ONLY a JSON array in this exact format, one entry per word, same order:
[
  {{"french": "maison", "english": "house; home", "pos": "noun", "gender": "f"}},
  {{"french": "parler", "english": "to speak; to talk", "pos": "verb", "gender": null}},
  {{"french": "grand", "english": "big; tall; great", "pos": "adjective", "gender": null}}
]
...
- No markdown fences, no commentary"""
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a French-English translator. Return only valid JSON arrays.",
            },
...
for level in ALL_LEVELS:
    ...
    for i in range(0, len(sl), TRANSLATE_BATCH):
        ...
        batch_words = [w["french"] for w in batch]
        batch_freqs = {w["french"].lower(): w["frequency"] for w in batch}
        ...
        for t in translated:
            t["frequency"] = batch_freqs.get(t["french"].lower(), 0)
