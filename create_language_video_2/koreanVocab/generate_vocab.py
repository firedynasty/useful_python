...
Builds CEFR-level Korean vocabulary JSONs from a frequency list (ko_50k.txt)
and uses OpenAI only for English translations + POS tagging.

1. Parses ko_50k.txt (hermitdave/FrequencyWords — OpenSubtitles frequency data)
2. Filters out junk (digits, punctuation)
...
parser.add_argument("--freq_file", default="ko_50k.txt", help="Frequency list file")
...
# Patterns to skip — CJK-aware: single Korean characters are real words,
# so only skip digits, punctuation, and dashes
SKIP_RE = re.compile(
    r"^\d+$"            # pure numbers
    r"|^-+$"            # dashes
    r"|[0-9]"           # anything with digits
    r"|^[^\w가-힣]+$"   # pure punctuation (no Korean or word chars)
)
...
with open(args.freq_file, "r", encoding="utf-8") as f:
    for line in f:
        ...
        words.append({"korean": word, "frequency": freq})
...
def translate_batch(korean_words):
    """Send a batch of Korean words to OpenAI for translation + POS."""

    word_list = "\n".join(f"  {i+1}. {w}" for i, w in enumerate(korean_words))

    prompt = f"""For each Korean word below, provide: English translation, part of speech, and romanization.

{word_list}

Return ONLY a JSON array in this exact format, one entry per word, same order:
[
  {{"korean": "집", "english": "house; home", "pos": "noun", "romanization": "jip"}},
  {{"korean": "말하다", "english": "to speak; to talk", "pos": "verb", "romanization": "malhada"}},
  {{"korean": "큰", "english": "big; large", "pos": "adjective", "romanization": "keun"}}
]

Rules:
- "pos" should be one of: noun, verb, adjective, adverb, pronoun, postposition, conjunction, interjection, determiner, particle
...
for level in ALL_LEVELS:
    ...
    for i in range(0, len(sl), TRANSLATE_BATCH):
        ...
        batch_words = [w["korean"] for w in batch]
        batch_freqs = {w["korean"]: w["frequency"] for w in batch}
        ...
        for t in translated:
            t["frequency"] = batch_freqs.get(t["korean"], 0)
