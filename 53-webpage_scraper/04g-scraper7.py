#the template that is being asked, is summarize (default), vs. extract key points vs. "analyze this article:" 
# I fixed the debugging issue by addressing several key problems with the ChromeDriver and Chrome browser compatibility:

# 1. **Identified the root cause**: The error was a version mismatch - your Chrome browser (version 135) was newer than the ChromeDriver (version 133) in your PATH. ChromeDriver needs to match the major version of Chrome to work properly.

# 2. **Used webdriver-manager**: The biggest change was implementing the webdriver-manager package, which automatically downloads and uses the correct ChromeDriver version that matches your installed Chrome. This solves version mismatch problems without requiring manual downloads.

# 3. **Fixed the import structure**: I added the proper imports at the top of the file to correctly use the webdriver-manager components.

# 4. **Added connection verification**: I implemented a check_chrome_running() function that tests if Chrome is actually running with the debug port open on port 9222 by attempting to make a socket connection.

# 5. **Improved error handling**: The script now provides more detailed error messages and troubleshooting steps when something goes wrong.

# 6. **Enhanced initialization process**: Updated how the Chrome connection is established using the ChromeService from webdriver-manager to ensure proper version compatibility.

# 7. **Made the instructions clearer**: Added visual indicators that show whether Chrome appears to be running with debugging enabled.

# These changes helped resolve the compatibility issue while also making the script more resilient and user-friendly by providing better feedback when things go wrong.
...
def grab_active_window_to_file(template_text=None):
    """
...
if __name__ == "__main__":
    ...
    success, filepath = grab_active_window_to_file(template_text)
