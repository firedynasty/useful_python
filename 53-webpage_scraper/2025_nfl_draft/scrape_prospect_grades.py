...
2. Copies the search term to your clipboard
3. You paste it into Google in Chrome, navigate to their NFL.com prospect page
4. Press Enter and the script reads the grade from the open page
...
def main():
    ...
    # 1. Load PFR draft data from saved progress or require it exists
    # If you've already scraped PFR data, load from the progress file
    # Otherwise, connect to Chrome to scrape it
    try:
        ...
        table_html = driver.execute_script("""
            var el = document.getElementById('drafts') || document.getElementById('draft');
            if (!el) {
                var tables = document.querySelectorAll('table');
                for (var i = 0; i < tables.length; i++) {
                    if (tables[i].querySelector('th[data-stat="player"]')) {
                        el = tables[i];
                        break;
                    }
                }
            }
            if (!el) throw new Error('Draft table not found on page. Check the URL loaded correctly.');
            return el.outerHTML;
        """)
    ...
    # Filter to rounds 1-4
    draft_r1_3 = [d for d in draft_data if d.get("draft_round", "") in ("1", "2", "3", "4")]
    print(f"Rounds 1-4: {len(draft_r1_3)} players\n")
    ...
    for i, d in enumerate(draft_r1_3):
        ...
        search_term = f"{player} nfl prospect site:nfl.com"
        pyperclip.copy(search_term)
        print(f"\n[{i+1}/{len(draft_r1_3)}] Search for: {search_term}  (copied to clipboard)")
        response = input("  Enter grade (s=skip, q=quit): ").strip().lower()
        ...
        grades[player] = response
        print(f"  -> Grade: {response}")
