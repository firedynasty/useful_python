    ...
    try:
        ...
        if "localhost:3000" in current_url:
            ...
            if chapter_content_match:
                # This will extract only the verses content
                book_name = chapter_content_match.group(1)
                plain_text = chapter_content_match.group(2).strip()
                print(f"Extracted content from {book_name}")
            else:
                # Fallback to find any chapter heading
                fallback_match = re.search(r'## ([A-Za-z ]+) \d+\n([\s\S]*)', plain_text)
                if fallback_match:
                    book_name = fallback_match.group(1)
                    plain_text = fallback_match.group(2).strip()
                    print(f"Used fallback extraction for {book_name}")
            
            # No need to add metadata
            full_text = plain_text
        else:
            # For other websites, include metadata
