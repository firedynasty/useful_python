...
def grab_active_window_to_file(template_text=None, copy_to_clipboard_flag=False):
    ...
    try:
        ...
        # Get all tabs and show detailed info about each
        all_tabs = browser.window_handles
        print(f"Found {len(all_tabs)} tabs")
        
        # Show info about all tabs first
        print("\nAll tabs found:")
        ...
        # Ask user which tab to scrape
        while True:
            try:
                tab_choice = input(f"\nWhich tab do you want to scrape? (1-{len(all_tabs)}): ")
                tab_number = int(tab_choice)
                if 1 <= tab_number <= len(all_tabs):
                    selected_tab_index = tab_number - 1  # Convert to 0-based index
                    break
                else:
                    print(f"Please enter a number between 1 and {len(all_tabs)}")
            except ValueError:
                print("Please enter a valid number")
        
        print(f"\nSwitching to tab {tab_number}...")
        browser.switch_to.window(all_tabs[selected_tab_index])
        ...
        # Show first 200 characters of the page content to debug
        page_text_preview = browser.find_element("tag name", "body").text[:200]
        print(f"\nFirst 200 characters of page content:")
        print(f"'{page_text_preview}...'")
        print()
