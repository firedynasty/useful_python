# This new code builds upon the previous multi-tab version with additional clipboard-related functionality and more output options. Here's how it compares to the previous multi-tab code:
# Key Differences

# Enhanced Clipboard Options:

# The copy_to_clipboard() function now has an append parameter that allows content to be added to existing clipboard content rather than replacing it
# This enables concatenating content from multiple tabs into a single clipboard operation


# New Output Options:

# Option 4: "Copy all tabs to clipboard iteratively (concatenated)" - combines all tab contents in clipboard with separators
# Option 5: "Copy all tabs to clipboard with summarization prompt" - same as option 4 but adds an AI prompt at the end requesting a summary of all articles


# Summarization Prompt:

# When using option 5, the script adds this text to the end of the clipboard content:
# "Please summarize all the articles above. Identify key themes and connections between them."
# This is specifically designed to help users who want to ask an AI to analyze multiple articles at once



# The Evolution of These Scripts
# Looking at all three versions we've seen:

# Original Version: Single-tab focus, basic clipboard functionality
# Second Version: Added multi-tab support and file output options
# This Version: Enhanced clipboard handling with content concatenation and AI prompting

# Use Case
# This latest version is clearly designed for users who want to:

# Extract content from multiple web pages
# Combine them into a single document
# Feed them to an AI tool with a specific prompt

# The addition of the summarization prompt at the end (option 5) shows that the developer is specifically thinking about using this tool with AI services that can analyze multiple articles at once and draw connections between them.
# This makes the tool particularly useful for researchers, analysts, or anyone who needs to compare and synthesize information from multiple web sources, possibly using AI tools like Claude to help with the analysis.
...
def copy_to_clipboard(text, append=False):
    """
    Copy text to clipboard
    
    Args:
        text: Text to copy
        append: If True, append to existing clipboard content instead of replacing
    """
    try:
        if append:
...
def main():
    ...
    print("4. Copy all tabs to clipboard iteratively (concatenated)")
    print("5. Copy all tabs to clipboard with summarization prompt")
    
    output_choice = input("Enter your choice (1-5): ")
    ...
    if output_choice in ('2', '3') and active_tab_only:
        ...
        pyperclip.copy('')
        print("\nCopying all tabs to clipboard (concatenated)...")
        
        # Copy each tab's content to clipboard, appending each time
        count = 0
        for title, content in tab_contents.items():
            if copy_to_clipboard(content, append=(count > 0)):
                count += 1
                print(f"✓ Added tab: {title}")
        
        print(f"\nSuccessfully copied {count} tabs to clipboard")
        
    elif output_choice == '5':
        # Clear clipboard first
        pyperclip.copy('')
        print("\nCopying all tabs to clipboard with summarization prompt...")
        ...
        # Add summarization prompt at the end
        if count > 0:
            final_prompt = "\n\nPlease summarize all the articles above. Identify key themes and connections between them."
            current = pyperclip.paste()
            pyperclip.copy(current + final_prompt)
            print(f"\nSuccessfully copied {count} tabs to clipboard with summarization prompt")
