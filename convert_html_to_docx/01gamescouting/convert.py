...
def extract_css_rules(soup):
    ...
    for style_tag in style_tags:
        ...
        # Simple CSS parser for table-related rules
        if 'table' in css_text or 'td' in css_text or 'th' in css_text:
            # Check if borders are defined in CSS
            if 'border:' in css_text or 'border-' in css_text:
                css_rules['has_table_borders'] = True
...
def add_html_table_to_doc(table_element, doc, css_rules=None):
    ...
    # 1. Explicit border attribute (border="1", etc.)
    # 2. CSS border styles
    # 3. Tables with <th> headers typically should have borders
    # 4. Check if CSS rules define borders
    # 5. Default to borders unless explicitly set to border="0"
    
    explicit_no_border = table_element.get('border') == '0'
    has_header_cells = table_element.find('th') is not None
    has_css_borders = (
        'border' in styles or 
        'border-collapse' in styles or
        (css_rules and css_rules.get('has_table_borders', False))
    )
    has_border_attribute = table_element.get('border', '') not in ['', '0']
    
    # Apply borders by default for tables with headers or unless explicitly disabled
    has_border = (
        has_border_attribute or 
        has_css_borders or 
        (has_header_cells and not explicit_no_border) or
        (not explicit_no_border and not table_element.has_attr('border'))
    )
...
def html_to_docx(html_file, output_file=None, encoding='utf-8'):
    ...
    # Extract CSS rules for better styling detection
    ...
    for element in body.children:
        ...
        if tag in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']:
            ...
            for child in element.children:
                if child.name == 'table':
                    add_html_table_to_doc(child, doc, css_rules)
                elif child.name in ['ul', 'ol']:
            ...
            add_html_table_to_doc(element, doc, css_rules)
...
def main():
    ...
    parser.add_argument('output', nargs='?', default='output.docx', help='Output DOCX file path (default: output.docx)')
