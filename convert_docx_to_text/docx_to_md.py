#!/usr/bin/env python3
"""
DOCX to Markdown converter that preserves document structure:
- Headings converted to Markdown headers (# ## ###)
- Tables converted to Markdown table format
- Lists preserved with bullets/numbers
- Bold/italic text preserved with Markdown syntax (**bold**, *italic*)
...
def format_run_text(run):
    ...
    if run._element.xpath('.//pic:pic'):
        # Try to get image description or use generic placeholder
        try:
            # Get the drawing element
            drawings = run._element.xpath('.//w:drawing')
            if drawings:
                # Try to get description from docPr element
                desc_elements = run._element.xpath('.//wp:docPr/@descr')
                if desc_elements and desc_elements[0]:
                    return f"[Image: {desc_elements[0]}]\n"
                # Try to get name
                name_elements = run._element.xpath('.//wp:docPr/@name')
                if name_elements and name_elements[0]:
                    return f"[Image: {name_elements[0]}]\n"
        except:
            pass
        return "[Image]\n"
...
def process_paragraph(paragraph):
    ...
    # Process regular paragraph with inline formatting (including images)
    formatted_text = ""
    for run in paragraph.runs:
        formatted_text += format_run_text(run)

    # If no content at all (no text, no images), skip the paragraph
...
def docx_to_md(docx_file, output_file=None, encoding='utf-8'):
    """
    Convert DOCX file to Markdown (.md) format.
    ...
    for block in iter_block_items(doc):
        if isinstance(block, Paragraph):
            ...
            table_text = format_table_to_markdown(block)
    ...
    if output_file is None:
        output_file = docx_file.rsplit('.', 1)[0] + '.md'
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert DOCX file to Markdown (.md) format'
    )
    ...
    try:
        docx_to_md(args.input, args.output, args.encoding)
