...
Auto-detects language from gloss CSV header (Chinese/Pinyin or Korean/Romanization).

For each pair:
  - Output english + romanization as 2 inserted rows
...
# ── Load preformatted CSV (auto-detect language) ─────────────────────────────
...
if "chinese" in gloss_header:
    lang = "chinese"
    native_col = gloss_header.index("chinese")
    rom_col = gloss_header.index("pinyin") if "pinyin" in gloss_header else 1
    csv_header = ["Chinese", "Pinyin", "English meaning"]
    verify_lang = "Chinese"
elif "hebrew" in gloss_header:
    lang = "hebrew"
    native_col = gloss_header.index("hebrew")
    rom_col = gloss_header.index("romanization") if "romanization" in gloss_header else 1
    csv_header = ["Hebrew", "Romanization", "English meaning"]
    verify_lang = "Hebrew"
else:
    lang = "korean"
    native_col = gloss_header.index("korean") if "korean" in gloss_header else 0
    rom_col = gloss_header.index("romanization") if "romanization" in gloss_header else 1
...
if not api_key:
    ...
    verify_prompt = f"""You are a {verify_lang} lyrics verification assistant.

I matched word gloss groups to lyric lines. Each group starts with an English
translation line and a romanization/pinyin line, followed by word glosses.
