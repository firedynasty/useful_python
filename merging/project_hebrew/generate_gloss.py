...
if "chinese" in header:
    ...
    rom_label = "Pinyin"
elif "hebrew" in header:
    lang = "hebrew"
    native_col = header.index("hebrew")
...
{verse['translation']}"""
    ...
    SYSTEM_PROMPT = """You are a Hebrew linguistics assistant that creates word-by-word glosses.

For each verse you receive, break down EVERY word. Return a JSON array of objects:
[
  {"hebrew": "בקרוב", "romanization": "bekarov", "english": "soon"},
  {"hebrew": "תזרח", "romanization": "tizrach", "english": "will rise"},
  ...
]

Rules:
- Use the romanization provided — do NOT re-romanize differently
- Include grammar notes in parentheses for prefixes, suffixes, and conjugations (e.g. ha-: the, -nu: our, -im: plural)
- Split compound prefixes when helpful (e.g. ba'rechovot → ba- + rechovot)
- Keep words in the same order as the verse
- Return ONLY the JSON array, no markdown, no explanation"""
...
{verse['translation']}"""
    ...
    json_keys = ("hebrew", "romanization", "english")
