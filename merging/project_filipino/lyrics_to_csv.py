"""
lyrics_to_csv.py (Filipino)

Reads a .txt file with Filipino lyrics (+ section headers like [Verse 1]).
Sends each verse to OpenAI to get English translation.

Outputs a CSV with 2 columns: Filipino, Translation
...
parser.add_argument("--output", default="filipino_table.csv", help="Path for output CSV")
...
SYSTEM_PROMPT = """You are a Filipino/Tagalog-to-English translation assistant. You will receive Filipino song lyrics.

For each line, provide an English translation.

Return a JSON array of objects, one per line:
[
  {"english": "I'm forcing myself to cope"},
  ...
]
...
for vi, verse in enumerate(all_verses):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": filipino_block},
        ],
        temperature=0,
    )
...
with open(args.output, "w", newline="", encoding="utf-8-sig") as f:
    writer = csv.writer(f)
    writer.writerow(["Filipino", "Translation"])
    seen_sections = set()
    for verse, results in zip(all_verses, all_results):
        if verse["section"] and verse["section"] not in seen_sections:
            writer.writerow([verse["section"], ""])
            seen_sections.add(verse["section"])
        filipino_cell = "\n".join(verse["lines"])
        trans_cell = "\n".join(r.get("english", "") for r in results[:len(verse["lines"])])
        writer.writerow([filipino_cell, trans_cell])
