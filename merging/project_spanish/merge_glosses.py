"""
merge_glosses.py (Spanish)

Walks through lyric pairs (adjusted txt) and gloss rows
(preformatted csv) in order with two pointers.

For each pair:
  - Output english + spanish as 2 inserted rows
  - Consume gloss rows until hitting one whose spanish text matches
    the pair's LAST spanish word
...
parser.add_argument("--output", default="spanish_output.csv", help="Path for output CSV")
...
native_col = gloss_header.index("spanish") if "spanish" in gloss_header else 0
eng_col = gloss_header.index("english meaning") if "english meaning" in gloss_header else 1
csv_header = ["Spanish", "English meaning"]

print(f"Detected language: Spanish")

rows = reader[1:]
section_headers = {"Verso 1", "Verso 2", "Verse 1", "Verse 2", "Pre-Chorus", "Chorus", "Coro", "Bridge", "Outro"}
...
if not api_key:
    ...
    verify_prompt = f"""You are a Spanish lyrics verification assistant.

I matched word gloss groups to lyric lines. Each group starts with an English
translation line and a Spanish line, followed by word glosses.
