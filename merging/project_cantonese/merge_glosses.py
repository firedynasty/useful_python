"""
merge_glosses.py (Cantonese)

Walks through lyric pairs (adjusted txt) and gloss rows
(preformatted csv) in order with two pointers.

For each pair:
  - Output english + Jyutping as 2 inserted rows
  - Consume gloss rows until hitting one whose Jyutping matches
    the pair's LAST Jyutping word (uses ENDSWITH for multi-token Jyutping)
...
parser.add_argument("--output", default="cantonese_output.csv", help="Path for output CSV")
...
native_col = gloss_header.index("cantonese") if "cantonese" in gloss_header else 0
rom_col = gloss_header.index("jyutping") if "jyutping" in gloss_header else 1
eng_col = gloss_header.index("english meaning") if "english meaning" in gloss_header else 2
csv_header = ["Cantonese", "Jyutping", "English meaning"]

print(f"Detected language: Cantonese")

rows = reader[1:]
section_headers = {"Verse 1", "Verse 2", "Pre-Chorus", "Chorus", "Bridge", "Outro", "Ending"}
...
for pair in lyric_pairs:
    ...
    # Peek ahead: does any gloss row's Jyutping match the END of this pair's romanization?
    found_ahead = False
    ...
    # Consume gloss rows until we hit one whose Jyutping matches the end of the line
...
if not api_key:
    ...
    verify_prompt = f"""You are a Cantonese lyrics verification assistant.

I matched word gloss groups to lyric lines. Each group starts with an English
translation line and a Jyutping line, followed by word glosses.
